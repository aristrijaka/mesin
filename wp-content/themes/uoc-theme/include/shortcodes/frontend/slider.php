...
if (!function_exists('cs_offerslider_shortcode')) {
    function cs_offerslider_shortcode($atts, $content = "") {
        ...
        if ($cs_offerslider_section_title && trim($cs_offerslider_section_title) !='' ) {
            $section_title    = '<div class="cs-section-title"><h2 class="'.$cs_offerslider_animation.'">'.esc_html($cs_offerslider_section_title).'</h2></div>';
        }
    ...
    if (function_exists('cs_short_code')) cs_short_code(CS_SC_OFFERSLIDER, 'cs_offerslider_shortcode');
}
...
if (!function_exists('cs_offerslider_item')) {
    function cs_offerslider_item( $atts, $content = null ) {
        ...
        if ( $cs_slider_image ) {
            ...
            $html    .='<img  src="'.esc_url($cs_slider_image).'" alt="'.cs_get_post_img_title($post->ID).'">';
            if ( $cs_readmore_link ) {
                $html    .='</a>';
            }
            $html    .='</figure>';
        ...
        if ( $cs_slider_title ) {
            $html    .='<h1>'.esc_html($cs_slider_title).'</h1>';
        }
        if ( $content ) {
        ...
        if ( $cs_readmore_link ) {
            ...
            $html    .='<a href="'.esc_url( $cs_readmore_link ).'"><button class="custom-btn cs-bg-color">'.esc_html($link_title).'</button</a>';
        }
        $html    .='</div>';
        $html    .='</div>';
        $html    .='</div>';
    ...
    if (function_exists('cs_short_code')) cs_short_code( CS_SC_OFFERITEM, 'cs_offerslider_item' );
}