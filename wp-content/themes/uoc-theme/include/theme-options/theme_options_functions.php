...
if ( ! function_exists( 'cs_options_backup_generate' ) ) {
	function cs_options_backup_generate() {
		...
		if ( ! $wp_filesystem->put_contents( $cs_filename, $cs_option_fields, FS_CHMOD_FILE) ) {
			echo __("Error saving file!", 'uoc' );
		} else {
			echo __("Backup Generated.", 'uoc' );
		}
...
if ( ! function_exists( 'cs_backup_file_delete' ) ) {
	function cs_backup_file_delete() {
		...
		if( is_file( $cs_filename ) ) {
			unlink( $cs_filename );
			printf( __("File '%s' Deleted Successfully", 'uoc' ), $file_name );
		} else {
			echo __("Error Deleting file!", 'uoc' );
		}
	...
	function cs_backup_file_restore() {
		...
		if( $file_path == 'yes' ) {
			...
			if( $cs_file_body != '' ) {
				...
				_e("File Import Successfully", 'uoc' );
			
			} else {
				_e("Error Restoring file!", 'uoc' );
			}
		...
		if( is_file( $cs_filename ) ) {
			...
			printf( __("File '%s' Restore Successfully", 'uoc' ), $file_name );
...
if ( ! function_exists( 'cs_reset_data' ) ) {
	function cs_reset_data(){
		global $reset_data,$cs_options;
			...
			if($value['type'] <> 'heading' && $value['type'] <> 'sub-heading' && $value['type'] <> 'main-heading') {
