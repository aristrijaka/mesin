...
if ( ! function_exists( 'cs_demo_importer' ) ) {
	...
	if(isset($_REQUEST['demo']) && $_REQUEST['demo']=='demo-data'){
		...
		if($cs_demoimport_error){
 			echo __( 'Error.', 'cs_frame' ) . '</p>';
			die();
		}else{
			
 			if( ! is_file( cs_framework::plugin_dir() . 'include/cs-importer/demo.xml')){
				echo '<p><strong>' . __( 'Sorry, there has been an error.', 'cs_frame' ) . '</strong><br />';
				echo __( 'The file does not exist, please try again.', 'cs_frame' ) . '</p>';
			}else{
				...
				if( ! get_option('cs_import_demo') ){
					...
					echo '<p><strong>' . __( 'You have already install this demo.', 'cs_frame' ) . '</strong><br />';
				}
			}
		}
	}
?>
   <div class="cs-demo-data">
        ...
        <div class="inn-text">
            <p><?php _e('Importing demo data helps to build site like the demo site by all means. It is the quickest way to setup theme. Following things happen when dummy data is imported;','cs_frame');?></p>
            <ul class="import-data">
                <li>&#8226;<?php _e('All wordpress settings will remain same and intact','cs_frame');?></li>
                <li>&#8226;<?php _e('Posts, pages and dummy images shown in demo will be imported','cs_frame');?></li>
                <li>&#8226;<?php _e('Only dummy images will be imported as all demo images have copy right restriction','cs_frame');?> </li>
                <li>&#8226;<?php _e('No existing posts, pages, categories, custom post types or any other data will be deleted or modified','cs_frame');?></li>
                <li>&#8226;<?php _e('To proceed, please click "Import Demo Data" and wait for a while','cs_frame');?></li>
                <li style="color:red;">&#8226; <b><?php _e('Note: Before Import demo data please make sure your server has following Setting','cs_frame');?></b></li>
                <li>
                	<ul style="margin-left:15px;">
                    	 <li>&#8226;<?php _e('post_max_size = 10M or Greater','cs_frame');?> </li>
                         <li>&#8226;<?php _e('upload_max_filesize = 10M or Greater','cs_frame');?> </li>
                         <li>&#8226;<?php _e('memory_limit = 128M or Greater','cs_frame');?></li>
                    </ul>
                </li>
                <li>&#8226;<?php _e('Your Current Setting','cs_frame');?> </b></li>
                <li>
                    <ul style="margin-left:15px;">
                    	<?php
...
function cs_update_themeoptions($page ='',$formdata = '',$theme_option = ''){
	if( isset( $page->ID ) ) {
		...
		echo '<p><strong>' . __( 'Theme Settings Saved.', 'cs_frame' ) . '</strong><br />';
